...
import { NIcon, NSelect, NTooltip, SelectGroupOption } from "naive-ui";
...
const resultParser = (apiOptions: Record<string, string>[], query: string = "") => {
    ...
    const originalMap = apiOptions.map(option => {
        ...
        if (includeCustom && query && optionLabel.toLowerCase().includes(queryTag)) includeCustom = false;
...
const handleSearch = debounce((query) => {
    ...
    axios.get(`${props.endpoint}?search=${query}`)
        .then(({ data }) => {
...
const fetchInitialValue = (id: string | number) => {
    ...
    axios.get(`${props.endpoint}/${id}`).then(({ data }) => {
        setOptions(resultParser([data?.data || data])
