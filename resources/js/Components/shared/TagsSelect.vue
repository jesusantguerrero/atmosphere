...
const updateTag = (label: any) => {
    if (label.id) {
        axios.post(`/api/labels/${label.id}`, {
            ...label,
...
const deleteTag = async (label: any) => {
    const canDelete = await ElMessageBox.confirm(`Are you sure you want to delete the tag: "${label.name}"?<br> This tag won't be deleted from already saved tasks`, "Delete Tag", {
        dangerouslyUseHTMLString: true,
    })

    if (label.uid && canDelete) {
        axios.delete(`/api/labels/${label.id}`).then(() => {
               ElNotification({
                message: "Tag deleted",
                type: "success"
            });
