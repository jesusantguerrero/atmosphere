...
<div class="text-gray-100 transition group" :class="[
  ...
  <div class="flex justify-between px-5 py-2">
    ...
    <div class="flex space-x-5">
        ...
        <div v-if="markAsPaid" class="font-bold text-primary-500 cursor-pointer" @click="$emit('paid-clicked')">
            Mark as Paid
        </div>
        <div v-if="markAsApproved" class="font-bold text-primary-500 cursor-pointer" @click="$emit('approved')">
            Approve
        </div>
        <div v-if="allowRemove" class="font-bold text-primary-500 cursor-pointer" @click="$emit('removed')">
            Remove
        </div>
    </div>
    </div>
