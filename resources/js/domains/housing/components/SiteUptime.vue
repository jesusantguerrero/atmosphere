...
<template>
    <article class="flex w-full items-center">
        <section class="grid w-full mt-2 bg-base-lvl-2 h-9 grid-cols-8 lg:grid-cols-30 place-items-end" :class="containerClass">
          <div v-for="(response, index) in lastResponses" :key="`${response}-${index}`" class="w-full h-full cursor-pointer active">
              <NPopover placement="bottom" trigger="hover">
                  ...
                  <p class="font-bold text-body-1">{{ formatDate(response)}}</p>
                  Event Happened
              </NPopover>
          </div>
        </section>
